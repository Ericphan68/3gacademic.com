...
import type { AdminTopupRow } from '@/server/services/topupService';

const STATUS: Record<string, { label: string; variant: 'warning' | 'success' | 'danger' }> = {
  PENDING: { label: 'Chờ xác nhận', variant: 'warning' },
  CONFIRMED: { label: 'Đã cộng ví', variant: 'success' },
  REJECTED: { label: 'Đã từ chối', variant: 'danger' },
};

export function TopupManager({ rows }: { rows: AdminTopupRow[] }) {
  ...
  const act = async (id: string, action: 'confirm' | 'reject') => {
    ...
    toast.success(action === 'confirm' ? 'Đã xác nhận & cộng ví' : 'Đã từ chối yêu cầu');
    router.refresh();
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2 text-sm text-[var(--color-muted)]">
        ...
        {pending > 0 ? `${pending} yêu cầu đang chờ xác nhận` : 'Không có yêu cầu nào đang chờ'}
      </div>
